...
public class GameControllerScript : MonoBehaviour
{
    ...
    private void updateClock()
    {
        ...
        else
        {
            ...
            if (imageClock2.fillAmount == 0)
            {
                ...
                if (GameManager.Instance.offlineMode)
                {
                    //showMessage("You " + StaticString.runOutOfTime);
                }
                else
                {
                    //showMessage(GameManager.Instance.nameOpponent + " " + StaticString.runOutOfTime);
                }
    ...
    public void showMessage(string message)
    {
        ...
        if (timeDiff > hideBubbleAfter + 1.0f)
        {
            ...
            if (!message.Contains(StaticString.waitingForOpponent))
                Invoke("hideBubble", hideBubbleAfter);
            else
            {
                waitingOpponentTime = StaticString.photonDisconnectTimeout;
    ...
    public IEnumerator updateMessageBubbleText()
    {
        ...
        if (!GameManager.Instance.opponentDisconnected)
        {
            if (!messageBubbleText.text.Contains("disconnected from room"))
                messageBubbleText.text = StaticString.waitingForOpponent + " " + waitingOpponentTime;
        }
